...
<template>
  <div v-if="game" class="max-w-3xl mx-auto px-md py-xl">
    <!-- Back link -->
    <NuxtLink
      to="/"
      class="inline-flex items-center gap-1.5 text-ui text-[--color-text-secondary] hover:text-[--color-brand] mb-lg group"
    >
    ...
    <!-- Photo gallery -->
    <div v-if="game.photos.length > 0" class="mb-lg rounded-[--radius-xl] overflow-hidden bg-[--color-surface-elevated]">
      <picture>
        <source
          type="image/webp"
    ...
    <div
      v-else
      class="mb-lg rounded-[--radius-xl] bg-[--color-surface-elevated] aspect-video flex items-center justify-center text-[--color-text-muted]"
    >
    ...
    <div class="flex items-start gap-3 mb-md">
      <div class="flex-1">
        <div v-if="game.featured" class="mb-2">
          <span class="px-2.5 py-1 text-tag font-semibold bg-[--color-brand] text-white rounded-full">
            Featured
          </span>
        ...
        <h1 class="text-detail-title font-bold text-[--color-text-primary]">{{ game.name }}</h1>
      </div>
    </div>

    <!-- Quick facts -->
    <div class="flex flex-wrap gap-md mb-lg pb-lg border-b border-[--color-border]">
      <div class="flex flex-col gap-0.5">
        <span class="text-meta font-medium text-[--color-text-muted] uppercase tracking-wide">Players</span>
        <span class="text-ui font-semibold text-[--color-text-primary]">{{ playerLabel(game.playerMin, game.playerMax) }}</span>
      </div>
      <div class="flex flex-col gap-0.5">
        <span class="text-meta font-medium text-[--color-text-muted] uppercase tracking-wide">Play time</span>
        <span class="text-ui font-semibold text-[--color-text-primary]">{{ timeLabel(game.timeMin, game.timeMax) }}</span>
      </div>
    </div>

    <!-- Tags -->
    <div v-if="game.tags.length > 0" class="flex flex-wrap gap-sm mb-lg">
      <span
    ...
    <!-- Description -->
    <p v-if="game.description" class="text-body text-[--color-text-primary] leading-relaxed">
      {{ game.description }}
    </p>
    ...
    <div
      ...
      class="grid grid-cols-3 gap-sm mt-lg"
    >
      <div
