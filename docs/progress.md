...
- Already-fine pages untouched (login, users, PhotoUpload, TagTypeahead, BggSearch). Build + unit green; e2e expected green (desktop renders the table/nav; the table's Edit link is first in DOM so `getByRole('link',{name:'Edit'}).first()` still resolves visibly).
