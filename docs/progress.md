...
**Mobile audit of staff pages (done, not yet implemented).** A staff member needs to add/edit/remove games from a phone. Audit results: **(c) significant rework** — `staff.vue` header nav (no wrap/hamburger, overflows < 640px) and `staff/index.vue` games table (Featured toggle + action buttons overflow horizontally). **(b) minor** — `GameForm.vue` player/time 2-col subgrids don't stack (`grid-cols-2` always), add/edit wrapper padding (`p-6`) tight on small phones, `tags/index.vue` rename form lacks `flex-wrap`. **(a) already OK** — login, users, PhotoUpload, TagTypeahead, BggSearch. See next-actions for the plan decision.
